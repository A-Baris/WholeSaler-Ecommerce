...
namespace WholeSaler.Web.FluentValidation.Validators.Product
{
    public class TelevisionValidator:AbstractValidator<Television>
    {
        ...
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("Name can not be empty or null").NotNull().WithMessage("Name can not be empty or null").MinimumLength(3).WithMessage("Minimum 3 characters");
            RuleFor(x => x.Color).NotEmpty().WithMessage($"Color can not be empty or null").NotNull().WithMessage("Color can not be empty or null").MinimumLength(3).WithMessage("Minimum 3 characters");
            RuleFor(x => x.Brand).NotEmpty().WithMessage($"Brand can not be empty or null").NotNull().WithMessage("Brand can not be empty or null").MinimumLength(3).WithMessage("Minimum 2 characters");
